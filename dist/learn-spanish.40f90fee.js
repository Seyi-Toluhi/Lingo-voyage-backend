...
let answerDisplay, correctAnswer, loginEmail, loginPassword, username, score = 0, questionsCount = 12, XP;
let selectionArr = [];
let randomSaved = {};
...
function attachEventListeners() {
    if (elements.login_button) elements.login_button.addEventListener("click", redirectToLogin);
    if (elements.activity_div) for(let i = 0; i < elements.activity_selection_buttons.length; i++){
        selectionArr.push(elements.activity_selection_buttons[i]);
        selectionArr[i].addEventListener("click", loadActivity);
    }
    if (elements.signup_button) elements.signup_button.addEventListener("click", showSignupForm);
    if (elements.check_answer) elements.check_answer.addEventListener("click", checkAnswer);
    if (elements.next_quiz) elements.next_quiz.addEventListener("click", skipQuiz);
    if (elements.logout_button) elements.logout_button.addEventListener("click", logOut);
    if (contentParam === "login") handleLoginContent();
    if (elements.speech_practice_answers) elements.speech_practice_answers.forEach((answer)=>answer.addEventListener("click", checkSelectedAnswer));
...
function handleLoginContent() {
    if (isLoggedIn()) //elements.login_form.classList.add("hide");
...
function displayUserInfo() {
    ...
    if (elements.login_form) elements.login_form.classList.add("hide");
    if (elements.signup_info) elements.signup_info.classList.add("hide");
    if (elements.profile_display_container) elements.profile_display_container.classList.remove("hide");
    ...
    const XPDisplayDiv = document.querySelector("#display_level");
    ...
    scoreDisplay.innerHTML = `Your Quiz Score is: ${userScore}`;
// }
}
function acctSetup(e) {
    ...
    if (!userExists) {
        ...
        redirectToLoginPage();
    } else alert("An account already exists with this email address");
    clearInputFields();
...
function handleLoginFormSubmit(event) {
    ...
    if (loginEmail === retrievedForm.email && loginPassword === retrievedForm.password) {
...
function getUserForm(email) {
    ...
    if (userForm) return JSON.parse(userForm);
}
function checkUserForm(email) {
    ...
    if (retrievedForm) {
        ...
        XP = retrievedForm.XP;
        console.log(XP);
    } else alert("Please create a user account");
}
function logOut() {
...
/*const generateRandom = (size) => {
  let randomIndex;
    ...
    let randomIndex;
    if (Object.keys(randomSaved).length < size) while(true){
    ...
    else //i need to call a function that says quiz completed
    displayQuizResult();
};
...
async function fetchQuiz() {
    const requestURL = "https://raw.githubusercontent.com/Seyi-Toluhi/language_learning_app/main/Language_learning/Spanish.json";
...
function displayQuiz(response) {
    const optionsArr = [
        ...
        "c",
        "d"
    ]; //this array gives me the keys to my quiz answers
...
*/ function checkAnswer() {
    let selectedAnswer;
    for (const radioButton of elements.radio_buttons)if (radioButton.checked) {
    ...
    if (selectedAnswer) {
        ...
        } else wrongAnswerDisplay();
    } else alert("Please pick your answer");
}
...
function nextQuiz() {
    ...
    if (answerDisplay) elements.game_progress.removeChild(answerDisplay);
}
function skipQuiz() {
    fetchQuiz();
}
...
function displaySpeechQuestion() {
    if (correctSpeechAnswer < 5) {
        ...
        for(let i = 0; i < speechPracticeOptions.length; i++)elements.speech_practice_answers[i].textContent = speechPracticeOptions[i];
    } else {
...
//# sourceMappingURL=learn-spanish.40f90fee.js.map
